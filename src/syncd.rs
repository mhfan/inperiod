...
file.write_all(b"/// XXX: refer to AtomicWeight::Abridged for parse and display
#[derive(Debug)] struct ConciseMeasure {
    ...
    'outer: for line in content.split('\n')
        ...
        let vstr = vstr.replace(' ', "_").replace("...", "");
        ...
        let (prec, uncer) = if ustr != "(exact)" {
            ...
            let vstr = vstr.replace('_', "");
            let dpos  = vstr. find('.').unwrap_or(0);
            let epos  = vstr.rfind('e').unwrap_or(vstr.len());
            ((epos - dpos - 1) as u8, 0)
        };
